(function () {
			...
			return t
				? new Promise(e => {
						...
						const r = `<script nonce=${
							...
							xe ? 'true,true' : `'x',b('${Ue}')`
						}, ${me ? `b('${Ee}'.replace('x',b('','text/css')))` : 'false'}, ${
							he ? `b('${Pe}'.replace('x',b('{}','text/json')))` : 'false'
				...
				: Promise.all([
						xe || ye(createBlob(Ue)).then(() => (Ce = true), noop),
						me &&
							ye(createBlob(Ee.replace('x', createBlob('', 'text/css')))).then(
								() => (Le = true),
								noop
							),
						he &&
	...
	function b(e, t) {
		...
		for (;;) {
			...
			if (e === t) break;
			92 === e
				? ((r += He.slice(s, Fe)), (r += l()), (s = Fe))
				: (8232 === e || 8233 === e || (u(e) && o()), ++Fe);
		}
	...
	function o() {
		throw Object.assign(
			Error(
				`Parse error ${qe}:${He.slice(0, Fe).split('\n').length}:${
					Fe - He.lastIndexOf('\n', Fe - 1)
				}`
			),
			{ idx: Fe }
