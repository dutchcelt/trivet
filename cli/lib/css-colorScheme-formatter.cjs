...
const { colorSchemeExtension } = require(path.join(__dirname, 'defaults.cjs'));
 ...
 * @returns {Object}
 */
const getExtension = token => token.$extensions[`${colorSchemeExtension}`];
...
const hasExtension = token => {
	...
	const extScope = ext && ext[`${colorSchemeExtension}`];
	return !!extScope;
};
 ...
 */
module.exports = {
	'css/colorscheme': function({ dictionary, options }) {
		...
		const str = allTokens
			...
			.map((/** @type{Object} */ token) => {
				...
				const cssColorScheme = getExtension(token);
				let cssString = '';
				if (cssColorScheme.mode?.light) {
